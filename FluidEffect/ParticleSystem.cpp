...
HRESULT ParticleSystem::Render(ID3D11DeviceContext* deviceContext, const Camera& camera)
{
    ...
    for (Particle* particle : m_particles)
    {
        particle->CalculateNearestParticles(m_particles, camera);
...
void ParticleSystem::Update(float deltaTime)
{
	...
	else if (!input.Held(VK_SHIFT) && input.Pressed('E') && GetParticleSpawner()->m_spawnRate < 40.0f)
		GetParticleSpawner()->m_spawnRate++;

	if (input.Held(VK_SHIFT) && input.Pressed('Q') && GetParticleSpawner()->m_velocity > 2.0f)
		GetParticleSpawner()->m_velocity--;
	else if (!input.Held(VK_SHIFT) && input.Pressed('Q') && GetParticleSpawner()->m_spawnRate > 2.0f)
		GetParticleSpawner()->m_spawnRate--;
    ...
    for (Particle* particle : m_particles)
    {
        ...
        if (particle->GetTimeToLive() <= 0.0f)
        {
    ...
    {
        auto iterator = std::find(m_particles.begin(), m_particles.end(), particle);
        if (iterator != m_particles.end())
        {
            m_particles.erase(iterator);
            delete particle;
        }
    }
}
